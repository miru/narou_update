...
send_notification () {
    FLG=NG
    RES=`egrep "(DL開始|第[0-9]+部分.*\(新着\)|完結したようです|の更新はキャンセルされました)" $2 | perl -pe 's/ID:[0-9]+　(.*) のDL開始/\n\1/g' | perl -pe 's/ID:[0-9]+　(.*) の(DL開始|更新はキャンセルされました)/\n\1/g'`
    while [ $FLG = "NG" ]
    do
        case "$NOTIFY_TYPE" in
            "PUSHBULLET")
...
send_notification_slack () {
    BODY="$2"
    curl -X POST --data-urlencode "payload={\"channel\": \"$SLACK_CHANNEL\", \"username\": \"narou_update\", \"text\": \"$1
    $BODY\", \"icon_emoji\": \":$SLACK_ICON:\"}" $SLACK_WEBHOOK
}
